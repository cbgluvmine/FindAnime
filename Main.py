...
def GetWebPageNum():
    ...
    num = input("\nHow many webpages do you want to search (1 - 1000) or choose 0 for a random num ")
    check = True
    while check:
        # Keeps checking if the str is a number
        while not num.isnumeric():
            num = input("\nPlease choose a number between (1 - 1000) or choose 0 for a random num ")
            check = False
        # Checks if the number is 0 which means that the computer chooses the number
        if int(num) is 0:
            num = randint(1, 1000)
        ...
        if int(num) > 1000 or int(num) < 1:
            ...
            # Checks if the number is a number and in-bounds)
            # Keeps checking if the str is a number
        if num.isnumeric() and int(num) <= 1000 and int(num) >= 1:
            return num
